...
class ReviewsResourcesGetListProcessor extends modObjectGetListProcessor
{
    ...
    public function prepareQueryBeforeCount(xPDOQuery $criteria)
    {
        ...
        $criteria->rightjoin('ReviewsReview', 'Review', 'Resource.id = Review.resource_id');
    ...
    public function prepareRow(xPDOObject $object)
    {
        return array_merge($object->toArray(), [
            'pagetitle_formatted' => $object->get('pagetitle') . ($this->modx->hasPermission('tree_show_resource_ids') ? ' (' . $object->get('resource_id') . ')' : '')
        ]);
    }
}
